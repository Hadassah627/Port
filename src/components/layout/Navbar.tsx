...
export const Navbar = () => {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-white/10 bg-white/85 backdrop-blur-xl dark:bg-ink-950/85">
      <Link to="/" className="absolute left-0 top-0 flex h-full items-center gap-3 px-4 py-4 sm:px-6 lg:px-8">
        <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-ink-700 text-sm font-bold text-gold-300 shadow-glow">
          AD
        </div>
        <div>
      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between pl-4 pr-0 py-4 sm:pl-6 sm:pr-0 lg:pl-8 lg:pr-0">
          ...
          {menuItems.map((item) => (
            <NavLink
              ...
              className={({ isActive }) => [
                ...
                isActive || activePath === item.to ? 'bg-ink-700 text-white' : 'text-ink-600 hover:bg-ink-100 dark:text-white/75 dark:hover:bg-white/10',
              ].join(' ')}
            >
              {item.label}
            </NavLink>
      ...
      {menuOpen ? (
        <div className="border-t border-white/10 bg-white pl-4 pr-0 py-4 shadow-xl dark:bg-ink-950 lg:hidden">
            ...
            {menuItems.map((item) => (
              <NavLink
                ...
                className={({ isActive }) => [
                  ...
                  isActive || activePath === item.to ? 'bg-ink-700 text-white' : 'bg-ink-50 text-ink-700 dark:bg-white/5 dark:text-white/80',
                ].join(' ')}
              >
                {item.label}
              </NavLink>
